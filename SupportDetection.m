...
% fprintf('\n');
fprintf('detected(total=%2d, good=%2d, bad=%2d, miss=%2d), RelErr=%4.2e',...
nnz(nzs), nnz(good_nzs), nnz(bad_nzs), nnz(miss_nzs), relerr2);
fprintf('\n');

figure;
% box on
h = stem(1:n,x,'k*','MarkerSize',5);
set(h, 'LineWidth', 2);
set(gca, 'FontSize', 14);
hold on;
h = stem(find(good_nzs),xbar(good_nzs),'ro','MarkerSize',10);
set(h, 'LineWidth', 2);
set(gca, 'FontSize', 12);
if any(bad_nzs)
	h = stem(find(bad_nzs), xbar(bad_nzs),'bx','MarkerSize',10);
	legend('true signal','true nonzero','false nonzero','Location','best');
    set(h, 'LineWidth', 2);
    set(gca, 'FontSize', 12);   
else
	legend('true signal','true nonzero','Location','best');
end
...
% nnz(true_nzs), nnz(nzs), nnz(good_nzs), nnz(bad_nzs), nnz(miss_nzs), relerr2),...
% 'fontsize',14);
title(sprintf('detected(total=%2d, good=%2d, bad=%2d, miss=%2d), RelErr=%4.2e',...
nnz(nzs), nnz(good_nzs), nnz(bad_nzs), nnz(miss_nzs), relerr2),...
'fontsize',14);
fprintf('\n'); 
